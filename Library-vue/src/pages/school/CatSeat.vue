...
<template>
  <div class="CatSeat">
            ...
            >
              ...
              <template slot-scope="scope">
                ...
                <el-button
                    size="mini"
                    type="primary"
                    @click="viewRoom(scope.$index, scope.row)"
                >查看座位
                </el-button
                >
                <!--                <el-button-->
    ...
    <!-- 座位编辑 -->
    <!-- Adjust the template to conditionally show the Edit component -->
    <Edit v-if="showEditPage" :roomId="roomId" @back="backFromEdit"></Edit>
  </div>
</template>
...
import EditSeat from "@/pages/school/EditSeat.vue"; // 确保路径正确

export default {
  name: "CatSeat",
  components: {
    EditSeat,
    Edit  // 添加这行来注册 Edit 组件
  },
  props: [],
  data() {
    //这里存放数据
    return {
      searchValue: "",
      libraryId: "",
      dialogFormVisible: false,  // 初始状态设置为 false，表示对话框未显示
      rooms: [],
      // 对话框
      showEditPage: false,
      formIdx: -1,
  ...
  methods: {
    ...
    backFromEdit() {
      this.showEditPage = false; // 假设这是控制显示 Edit 组件的标志
    },
    ...
    viewRoom(index, row) {
      ...
      this.editRoomTag = false;  // Assuming you have another flag for Edit component
      // Set a new flag or use an existing one to control the display of the Edit component
      this.showEditPage = true;  // You need to define this in data() and handle its usage
    },
    // 返回到房间管理
    // 子组件调用
    back() {
